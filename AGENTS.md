...
A Node / Express JSON API with a `/health` route; it listens on the port Dockhold assigns. Deploy target: Dockhold (https://dockhold.eu).
...
2. `npx dockhold deploy --name express-api-starter` from this folder.
...
- The Deploy button link in README.md opens the Dockhold dashboard with the repo and name pre-filled; the user signs in, confirms, and the app builds: https://app.dockhold.eu/new?repo=https://github.com/dockhold/express-api-starter&name=express-api-starter&ref=button
...
- A database is optional. Add `--db` to the deploy (or `with_database: true`) only if the user wants one; the app then reads `DATABASE_URL`.
